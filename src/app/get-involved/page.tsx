      ...
      >
        <header className="py-20 md:py-32 text-center relative">
          <div
            ...
            style={{
              backgroundImage:
                "radial-gradient(var(--tw-color-slate-300) 0.5px, transparent 0.5px)",
              backgroundSize: "15px 15px",
            }}
          ></div>
          <motion.h1
            variants={heroTitleVariants}
            className="text-3xl md:text-6xl lg:text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-emerald-600 via-teal-500 to-sky-600 mb-6 relative z-10 px-4"
          >
            {pageTitleString}
          </motion.h1>
          <motion.p
            variants={heroSubtitleVariants}
            className="text-xl text-slate-700 max-w-3xl mx-auto px-6 relative z-10"
          >
            {pageSubtitle}
          </motion.p>
        </header>

        <motion.main variants={gridVariants} className="pb-20 md:pb-32 md:px-8">
            ...
            {involvementOpportunities.map((item) => (
              ...
              >
                ...
                >
                  <svg
                    viewBox="0 0 100 100"
                    className={`absolute inset-0 w-full h-full opacity-10 ${item.gradientFrom} ${item.gradientTo}`}
                  >
                    <motion.path
                      d={item.iconBgShape}
                  ...
                  <div
                    className={`absolute inset-0 bg-gradient-to-br ${item.gradientFrom} ${item.gradientTo} opacity-10 group-hover:opacity-20 transition-opacity duration-300 blur-xl`}
                  ></div>
                  <item.Icon
                    className={`relative z-10 w-20 h-20 md:w-24 md:h-24 text-${item.accentColor}-600 group-hover:text-${item.accentColor}-500 transition-colors duration-300`}
                    strokeWidth={1.5}
                  />
                </motion.div>
                ...
                >
                  <h2
                    className={`text-2xl md:text-3xl font-semibold mb-5 bg-clip-text text-transparent bg-gradient-to-r ${item.gradientFrom} ${item.gradientTo}`}
                  >
                    {item.title}
                  </h2>
                  ...
                  >
                    ...
                    <ChevronRight
                      size={22}
                      className="ml-2.5 transform transition-transform duration-300 group-hover:translate-x-1"
                    />
                  </motion.a>
                </motion.div>
              </motion.div>
